...
# Import components from your pipeline and model files
from pipeline import get_data_pipeline
from model_withAttention import Encoder, AttentionDecoder, Seq2Seq
...
def main():
    ...
    DEC_EMB_DIM = 256
    HIDDEN_DIM = 512
    NUM_LAYERS = 1  # Standard structure for Bahdanau attention comparison
    DROPOUT = 0.5
    CLIP = 1.0
    EPOCHS = 5
    
    # 3. Model instantiation
    print("Initializing Additive Attention Seq2Seq Architecture...")
    encoder = Encoder(INPUT_DIM, ENC_EMB_DIM, HIDDEN_DIM, NUM_LAYERS, DROPOUT)
    decoder = AttentionDecoder(OUTPUT_DIM, DEC_EMB_DIM, HIDDEN_DIM, NUM_LAYERS, DROPOUT)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if valid_loss < best_valid_loss:
            ...
            torch.save(model.state_dict(), 'attention_model_best.pt')
